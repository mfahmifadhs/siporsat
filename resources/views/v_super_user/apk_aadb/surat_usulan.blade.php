    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12 form-group" style="margin-right: 15%;margin-left: 15%;">
                ...
                <a href="{{ url('super-user/oldat/surat/pdf-bast/'. $dataUsulan->kode_otp_usulan) }}" rel="noopener" target="_blank" class="btn btn-danger pdf">
                    ...
                    <div class="row">
                        <div class="col-md-12 form-group">
                            <p class="m-0 text-capitalize">
                                Pengusul <span style="margin-left: 9%;"> : {{ $dataUsulan->nama_pegawai }} </span> <br>
                                Jabatan <span style="margin-left: 9.8%;"> : {{ $dataUsulan->jabatan.' '.$dataUsulan->tim_kerja }}</span> <br>
                                Unit Kerja <span style="margin-left: 8.5%;"> : {{ $dataUsulan->unit_kerja }}</span> <br>
                                Tanggal Usulan <span style="margin-left: 4.7%;"> : {{ \Carbon\Carbon::parse($dataUsulan->tanggal_usulan)->isoFormat('DD MMMM Y') }}</span> <br>
                                Rencana Pengguna <span style="margin-left: 2%;"> : {{ $dataUsulan->rencana_pengguna }}</span> <br>
                            </p>
                            ...
                            <table class="table table-bordered m-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $dataKendaraan->kategori_barang }}</td>
                                        <td>{{ $dataKendaraan->spesifikasi_barang }}</td>
                                        <td>{{ $dataKendaraan->jumlah_barang }}</td>
                                        <td>{{ $dataKendaraan->satuan_barang }}</td>
                                    </tr>
                                    @endforeach
                                    @else
                                    @foreach($dataUsulan->detailPerbaikan as $dataKendaraan)
                                    <tr>
                                        <td>{{ $no++ }}</td>
                                        <td>{{ $dataKendaraan->kode_barang }}</td>
                                        <td>{{ $dataKendaraan->nup_barang }}</td>
